...
const BookingListUser = ({ navigation }) => {
  ...
  useEffect(() => {
    console.log(orders.map((item) => item._id));
    console.log(otps.map((item) => item.order_id));
  }, []);
  ...
  const fetchOrders = async () => {
    ...
    if (data && data.token) {
      ...
      // console.log("order_id : " + otps.responseData.otps[0].order_id);
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            {item?.parkingOwnerDetails?.phoneNumber || "-"}
          </Text>
        </View>
        <View style={{ height: 10 }} />
